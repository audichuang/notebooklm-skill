...
用法:
  uv run scripts/episodic_podcast.py --config series_config.yaml [OPTIONS]
...
前置條件:
  - uv tool install notebooklm-py (CLI 隔離安裝)
...
try:
    ...
    print("❌ 缺少 pyyaml，請用 uv run 執行此腳本: uv run scripts/episodic_podcast.py --config ...")
    sys.exit(1)
...
範例:
  # 預覽模式
  uv run scripts/episodic_podcast.py --config series.yaml --dry-run

  # 生成全部
  uv run scripts/episodic_podcast.py --config series.yaml

  # 續製（從第 3 集開始）
  uv run scripts/episodic_podcast.py --config series.yaml --start 3

  # 加上知識蒸餾
  uv run scripts/episodic_podcast.py --config series.yaml --distill
        """,
    )
